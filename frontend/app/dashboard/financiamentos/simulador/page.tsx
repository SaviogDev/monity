...
export default function FinancingSimulatorPage() {
  ...
  return (
    <div className="p-6 lg:p-10 max-w-6xl mx-auto space-y-8">
      ...
      {/* HEADER */}
      <div className="bg-[var(--bg-card)] rounded-[2.5rem] p-8 shadow-xl border">
      ...
      {/* INPUT */}
      <div className="bg-[var(--bg-card)] rounded-[2.5rem] p-8 shadow-lg border grid grid-cols-1 md:grid-cols-2 gap-6">
        
        <Input label="Valor do bem" value={form.assetValue} onChange={(v: string)=>setForm({...form, assetValue:v})}/>
        <Input label="Entrada" value={form.downPayment} onChange={(v: string)=>setForm({...form, downPayment:v})}/>
        <Input label="Prazo (meses)" value={form.months} onChange={(v: string)=>setForm({...form, months:v})}/>
        <Input label="Taxa (% ao mês)" value={form.rate} onChange={(v: string)=>setForm({...form, rate:v})}/>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full p-4 rounded-2xl border bg-[var(--bg-surface)] font-bold text-lg"
      />
    </div>
  );
}
...
function ResultCard({ title, main, details, color }: any) {
  return (
    <div className="bg-[var(--bg-card)] rounded-[2rem] p-6 shadow-lg border">
